...
const consoleFormat = combine(
    ...
    printf(({ level, message, stack, timestamp, ...extra }) => {
        ...
        const extraString = Object.keys(extra).length ? ` ${Object.values(extra).join(' ')}` : '';
        return `${log}${extraString}`
    })
);
...
function createDailyRotateFileTransport(dirname: string, filename: string, level?: string): DailyRotateFile {
