...
import type { SeededLabels } from "@infra/prisma/seed/constructors/labels";
import type { SeededProgrammingLanguages } from "@infra/prisma/seed/constructors/programmingLanguages";

import {
    exerciseData,
    type ExerciseKey
} from "@infra/prisma/seed/data/exercises"
import { logSeedBody, logSeedHeader, prisma, readSeed } from "@infra/prisma/seed/utils";
...
async function seedExercises(
    ...
    const rows = await Promise.all(exerciseData.map(async data => {
        const exerciseFolderName = data.key.split(/(?=[A-Z])/).join('-').toLowerCase();
        const exerciseFolder = join('data', 'exercises', exerciseFolderName);
        ...
        for (const specificData of data.specificExercises) {
            const specificExercisePath = join(exerciseFolder, specificData.programmingLanguageKey);
            const srcPath = join(specificExercisePath, 'src');
            const testsPath = join(specificExercisePath, 'tests');
            const programazioLengoaia = seededProgrammingLanguages[specificData.programmingLanguageKey];
            ...
            const tests = await Promise.all(
                specificData.tests.map(async test => ({
                    ...
                    ordena: test.ordena,
                    pisua: test.pisua,
                    testa_kodea: await readSeed(testsPath, test.testa_kodea),
                    timeout: test.timeout
                }))
            );

            const headerFile = "buru_fitxategia" in specificData && specificData.buru_fitxategia
            ...
            await prisma.ariketaZehatza.upsert({
                create: {
                    ...
                    erreferentzia_emaitza: await readSeed(srcPath, specificData.erreferentzia_emaitza),
                    funtzio_izena: specificData.funtzio_izena,
                    hasierako_kodea: await readSeed(srcPath, specificData.hasierako_kodea),
                    programazio_lengoaia_id: programazioLengoaia.programazio_lengoaia_id,
            ...
            logSeedBody(`${ariketa.izenburua} - ${programazioLengoaia.izena} ${programazioLengoaia.bertsioa}`);
        }
