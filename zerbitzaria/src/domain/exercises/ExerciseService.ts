...
async function getExercise(
    ...
    const exerciseData = await ExerciseRepo.getExercise(data.ariketa_id, erabiltzailea_id, preferedProgrammingLanguage);
    if (!exerciseData) {
        throw new RequestError(HttpStatusCode.NOT_FOUND, 'Ariketa ez da aurkitu');
    }
    logger.info('Erabiltzaileak ariketa lortu du', { ariketa_id: data.ariketa_id, erabiltzailea_id, programazio_lengoaia_id: preferedProgrammingLanguage })
    return exerciseData;
}
...
async function getSpecificExercise(
    ...
    const exercise = await ExerciseRepo.getSpecificExercise(data.ariketa_id, erabiltzailea_id, data.programazio_lengoaia_id);
    if (!exercise) {
        throw new RequestError(HttpStatusCode.NOT_FOUND, 'Ariketa ez da aurkitu');
    }
    logger.info('Erabiltzaileak ariketa zehatza lortu du', { ariketa_id: data.ariketa_id, erabiltzailea_id, programazio_lengoaia_id: data.programazio_lengoaia_id })
    return exercise;
}
...
async function listExercises(
    ...
    const viewOnlyUserLevel = await UserRepo.getViewOnlyMyLevel(erabiltzailea_id);
    if (viewOnlyUserLevel) {
        const educationLevel = await UserRepo.getEducationLevel(erabiltzailea_id);
        if (educationLevel) {
    ...
    const exercises = await ExerciseRepo.getExercises(erabiltzailea_id, newFilters);
    logger.info('Erabiltzaileak ariketa zerrenda lortu du', { erabiltzailea_id })
    return exercises;
}
