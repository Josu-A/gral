...
async function login(
    ...
    if (!user.aktibatuta) {
        throw new RequestError(
            ...
            "Erabiltzaile hau ez dago aktibatuta",
        );
    }
    const accessToken = signAccessToken(
        user.erabiltzailea_id,
        user.helbide_elektronikoa,
    );
    const jti = createJti();
    ...
    logger.info("Erabiltzaileak saioa hasi du", {
        erabiltzailea_id: user.erabiltzailea_id,
        ip,
        userAgent,
    });
...
async function logout(refreshToken: string): Promise<void> {
    ...
    if (token && !token.iraungitutako_data) {
        ...
        logger.info("Erabiltzaileak saioa amaitu du", {
...
async function refreshToken(
    ...
    const refreshTokenHash = hashToken(refreshToken);
    const token = await AuthRepo.findRefreshToken(
        refreshTokenHash,
        decoded.jti,
    );
    ...
    if (!token) {
        ...
        logger.warn(
            "Tokena berrerabiltzen saiatu da, erabiltzailearen saio guztiak baliogabetu dira",
            { erabiltzailea_id: decoded.id, ip },
        );
        throw new RequestError(
            HttpStatusCode.UNAUTHORIZED,
            "Tokena baliogabea da",
...
async function register(data: IRegister): Promise<void> {
    ...
    if (existingUser) {
        throw new RequestError(
            HttpStatusCode.OK,
            "Erabiltzailea badago dagoeneko",
        );
    }
...
async function requestPasswordRestore(
    data: IRequestPasswordRestore,
): Promise<void> {
    const user = await AuthRepo.findUser(data.helbide_elektronikoa);
    if (user) {
        // TODO: Implement password restore request logic
        logger.info("Pasahitza berrezartzeko eskaera jaso da", {
            erabiltzailea_id: user.erabiltzailea_id,
        });
    }
}
