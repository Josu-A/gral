...
const LoginSchema = z.object({
    helbide_elektronikoa: z.email(),
    pasahitza: z.string().min(1)
});
...
const RegisterSchema = z.object({
    helbide_elektronikoa: z.email('Helbide elektroniko ez da baliozkoa').max(255),
    ikasketa_maila: z.enum(['Hasiberria', 'Ertaina', 'Aurreratua']),
    izena: z.string().min(1, 'Izena beharrezkoa da').max(255),
