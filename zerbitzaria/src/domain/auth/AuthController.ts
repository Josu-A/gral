...
async function refreshToken(
    ...
    if (!refreshToken) {
        res.status(HttpStatusCode.UNAUTHORIZED).json(
            formatError("Ez da freskatze tokenik aurkitu"),
        );
        return;
    }
...
async function requestPasswordRestore(
    ...
    res.status(HttpStatusCode.OK).json(
        formatSuccess({
            message:
                "Pasahitza berrezartzeko argibideak helbide elektronikora bidali dira, baldin eta erabiltzailea existitzen bada",
        }),
    );
}
